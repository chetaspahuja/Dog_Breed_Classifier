...
def adjust_results4_isadog(results_dic, dogfile):
    ...
    """           
    # Creating a dictionary before reading the dognames.txt
    dogsName_dic = dict() 
    
    # Read the contents of dogfile.txt to a dictionary(dogsname_dic)
    with open(dogfile,"r") as file:
        line = file.readline() 
        
        # processing each line until the end of file
        while line != "":
    ...
    for key in results_dic:
        # If pet label = is dog 
        if results_dic[key][0] in dogsName_dic:
            # if classifier label = is dog
            if results_dic[key][1] in dogsName_dic:
                results_dic[key].extend((1,1))
            else:
                # pet label = is dog, classifier label is not dog
            ...
            if results_dic[key][1] in dogsName_dic:
                ...
                # both pet label and classifier label are not
